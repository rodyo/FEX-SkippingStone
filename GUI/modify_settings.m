function varargout = modify_settings(funfcn, varargin)
    ...
    % get globals
    global MainWin
    global launch_tab sequence_tab arrival_tab algorithms_tab
    ...
    [ignored,funfcn]       = evalc(['@',funfcn]);        %#ok<ASGLU> convert to function handle;
    [varargout{1:nargout}] = feval(funfcn, varargin{:}); % NTOE: STR2FUNC() doesn't work in this context
    ...
    function [environment,...
              ...
              constants,...
              calculation,...
              settings] = default_values(rootdir) %#ok<DEFNU>
        ...
        % Default BG-color for edit boxes should be WHITE
        ...
        costfuns = dir(fullfile(rootdir, 'plugins', 'costfunctions'));
        names    = {costfuns.name};
        dirs     = [costfuns.isdir];
        costfuns = names(~dirs);
        costfuns = cellfun(@(x) x(1:end-2),...
                           costfuns,...
                           'UniformOutput', false);

        % get info on each costfunction
        for ii = 1:numel(costfuns)
            % do it carefully. If something goes wrong, make a report
            try
                % evaluate
                [dummy,costfun_info] = evalc(costfuns{ii});%#ok
                % check the info
                ...
                environment.plugin_info.costfuns(ii) = costfun_info;

            catch %#ok
                errordlg(['Error detected in costfunction plugin ''', costfuns{ii},...
                         ''': plugins must return ',...
                         'a structure with the fields ''name'', ''function_handle'', ',...
                         '''description'' and ''axis_label'' when called without ',...
                         'input arguments.'], ...
        ...
        postprocessors = dir(fullfile(rootdir, 'plugins', 'postprocessors'));
        names          = {postprocessors.name};
        dirs           = [postprocessors.isdir];
        postprocessors = names(~dirs);
        postprocessors = cellfun(@(x) x(1:end-2),...
                                 postprocessors,...
                                 'UniformOutput', false);

        % get info on each postprocessor
        for ii = 1:numel(postprocessors)

            % do it carefully. If something goes wrong, make a report
            try
                % evaluate
                [dummy, pp_info] = evalc(postprocessors{ii});%#ok

                % check the info & GUI commands
                if any(~isfield(pp_info, {'name'
                                          'function_handle'
                                          'GUI_function_handle'
                                          'plot_function_handle'}))
                    error(' '); %#ok<ERTAG> <--- force the try-catch to fail
                end

                % insert in output argument
                environment.plugin_info.postprocessors(ii) = pp_info;

            catch %#ok
                errordlg(['Error detected in post-processor plugin ''', postprocessors{ii},...
        ...
        constants.timeunits = {1
                               ...
                               30.471*86400
                               365.25*86400};
         % NOTE: (Rody Oldenhuis) cell array allows use of deal()
        ...
        oo.TolCon = 1e-6;
        oo.popsize = [];                               % non-standard options
    ...
    function change_all_settings() %#ok<DEFNU>
        ...
        set(lt.MaxC3Launch         , 'string', sl.max_C3            );
            % TODO: this should be changed when implementing launchers
        set(lt.MinPayloadMass      , 'string', sl.payload_mass      );
        set(lt.MinPayloadMassMargin, 'string', sl.payload_mass_margin);

        % power
        for ii = 1:2
            set(lt.power_radio(ii), 'value', sp.selected(ii)); end
        ...
        set(lt.NEP(5),  'string', sp.NEP.power_per_battery);
            % TODO: this should be changed when implementing different battery models
        ...
        % data
        for ii = 1:3
            set(ot.global_optimizer(ii), 'value', so.global.optimizer(ii)); end

        for ii = 1:2
            set(ot.LowThrustApproximation(ii), 'value', so.global.low_thrust_approximation(ii)); end

        for ii = 1:3
            set(ot.Ephemerides(ii), 'value', so.ephemerides(ii)); end
        ...
        elseif settings.optimize.local.optimizer
            ...
            for ii = 1:3
                set(ot.local_optimizer(ii), 'value', so.local.optimizer(ii)); end
        end

        for ii = 1:3
            set(ot.integrator(ii), 'value', so.local.integrator(ii)); end

        for ii = 1:2
            set(ot.low_thrust_optimization_method(ii), 'value', so.local.method(ii)); end
    ...
    function stdargout = change_single_setting(setting,...
                                               handle,...
                                               varargin) %#ok<DEFNU>
        ...
        % Call control-specific handler
        switch lower(type)

            % edit boxes
            case 'edit'
                new_setting = str2double(get(object, 'string'));

                if check_value(new_setting);
                    settings = assign_field(settings, setting, new_setting);
                else
                    % (error)
                    set(object, 'string', get_field(settings, setting));
                    return;
                end

            % check boxes, popups
            case {'checkbox' 'popupmenu'}
                settings = assign_field(settings, setting, get(object,'value'));

            % buttongroups
            case 'uipanel'
                which_one = (handle == get(object, 'selectedobject'));
                settings  = assign_field(settings, setting, which_one);
...
% Convert a string to an index that can be used as a structure index
% in setfield()/getfield(). 
%
% For example,
%
%   'field1.field2.field(3).field(2)'
%
% converts to 
%
%    I = {'field1', 'field2', 'field', {3}, 'field', {2}}
%
% so that 
%
%   getfield(S, I{:})
%
% retrieves the value at that field.
function fieldinds = string_to_fieldinds(str)
    
    
    uF = {'UniformOutput', false};
    
    % plain field names
    fieldinds  = regexp(str, '\.', 'split');
    
    % Some fields may have an index as well
    indexed_fields = cellfun(@(x) regexp(x, '([^\(]+)\((\d+)\)', 'tokens', 'once'), ...
                             fieldinds,...
                             uF{:});
    
    have_indfld = ~cellfun('isempty', indexed_fields);
    
    % If that is the case, we have to do a bit more work
    if any(have_indfld)
        
        % Convert index to a format that setfield() understands
        indexed_fields(have_indfld) = cellfun(@(x) {x{1} {str2double(x{2})}},...
                                              indexed_fields(have_indfld),...
                                              uF{:});
                             
        % weave both cell arrays together to form the correct
        % cell to use for indexing with getfield/getfield
        new_fieldinds = {};
        for ii = 1:numel(fieldinds)
            if have_indfld(ii)
                new_fieldinds = [new_fieldinds; 
                                 indexed_fields{ii}']; %#ok<AGROW>
            else
                new_fieldinds = [new_fieldinds; 
                                 fieldinds{ii}]; %#ok<AGROW>
            end
        end
        
        fieldinds = new_fieldinds;
        
    end
    
end
function S = assign_field(S, field, value)
    
    fieldinds = string_to_fieldinds(field);
    
    if numel(fieldinds) == 1
        % faster for simple cases
        S.(fieldinds{1}) = value;
    else
        % more versatile for more complex cases
        S = setfield(S, fieldinds{:}, value);
    end
...
function good = check_value(value)
    ...
    if ~isreal(value) % I've had a "+i" typo once...don't ask
        uiwait(errordlg('Value must be real.',....
                        'Complex value received'));
        uiresume;
        good = false;
        return;
    end
    ...
    if (value < 0)
        uiwait(errordlg('Value must be positive.',...

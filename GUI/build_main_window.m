function build_main_window(environment, model, constants, calculation, settings)
    ...
    handles.tab(launch_tab).panel = uipanel(...
        ...
        power_panel = uipanel(...
            ...
            set(power_group,...
                'SelectionChangeFcn', @(varargin) {...
                        modify_settings('change_single_setting',...
                                        'power.selected', ...
                                        handles.tab(launch_tab).power_radio,...
                                        varargin{:});
                        callbacks('change_power_supply',...
                                  varargin{:})...
                        }...
            );
            ...
            handles.tab(launch_tab).SEP(7) = uicontrol(...
                ...
                'Callback'  , @(varargin) {...
                        modify_settings('change_single_setting',...
                                        'power.SEP.jettisonable',...
                                        [],...
                                        varargin{:});
                        callbacks('enable_jettison',...
                                  varargin{:})...
                        }...
            );
                ...
                handles.tab(launch_tab).Jettison(1) = uicontrol(...
                    ...
                    'Background', environment.colors.edit_bgcolor,...
                    'Enable'    , 'off',...
                    'Position'  , [0.225250596658711 0.295275590551182 0.20673031026253 0.088740157480315],...
                    'Style'     , 'edit', ...
                    'callback'  , @(varargin)...
                        modify_settings('change_single_setting',...
                                        'power.SEP.panel_mass',...
                                        [],...
                                        varargin{:})...
                );
        ...
        handles.tab(launch_tab).powersupply_panel = power_panel;
        ...
        TE_panel = uipanel(...
            ...
            set(hilo,...
                'SelectionChangeFcn', @(varargin){...
                        modify_settings('change_single_setting', ...
                                        'propulsion.selected',...
                                        handles.tab(launch_tab).propulsion,...
                                        varargin{:});
                        callbacks('engine', ...
                                  varargin{:})...
                        }...
            ...
            handles.tab(launch_tab).Ion(8) = uicontrol(...
                ...
                'callback'  , @(varargin) ...
                        modify_settings('change_single_setting',...
                                        'propulsion.ion_engine.min_power',...
                                        [],...
                                        varargin{:})...
            );
            ...
            handles.tab(launch_tab).Sail(2) = uicontrol(...
                ...
                'callback'  , @(varargin) ...
                        modify_settings('change_single_setting', ...
                                        'propulsion.Solar_sail.surface_area',...
                                        [], ...
                                        varargin{:})...
            );
            ...
            handles.tab(launch_tab).Isp(1) = uicontrol(...
                ...
                'callback', @(varargin){...
                        modify_settings('change_single_setting', ...
                                        'propulsion.different_Isp.check',...
                                        [], ...
                                        varargin{:});
                        callbacks('captureIsp',...
                                  varargin{:})...
                        }...
            );
    ...
    handles.tab(sequence_tab).panel = uipanel(...
        ...
        mdl_panel = uipanel(...
            ...
            set(mdl_group,...
                'SelectionChangeFcn', @(varargin) {...
                        modify_settings('change_single_setting',...
                                        'model(1:3)', ...
                                        handles.tab(sequence_tab).selected_model(1:3),...
                                        varargin{:});
                        callbacks('model_selection', ...
                                  varargin{:})...
                        }...
            );

            % MPCORB
            % TODO: plugin code does not belong here
            ...
            handles.tab(sequence_tab).UpdateMPButton = uicontrol(...
                ...
                'callback', @(varargin) ...
                        callbacks('update_minor_planets_data',...
                                  true, ...
                                  varargin{:})...
            );
        ...
        constr_panel = uipanel(...
            ...
            handles.tab(sequence_tab).MinDistToSun = uicontrol(...
                ...
                'callback'  , @(varargin)...
                        modify_settings('change_single_setting', ...
                                        'GAM.constraints.min_Solar_distance',...
                                        [],...
                                        varargin{:})...
            );
            uicontrol(...
            ...
            handles.tab(sequence_tab).MaxTotalDeltaV = uicontrol(...
                ...
                'callback'  , @(varargin){...
                        modify_settings('change_single_setting', ...
                                        'GAM.constraints.max_DV',...
                                        [],...
                                        varargin{:});
                        callbacks('max_total_DV_check',...
                                  'warn_user', ...
                                  varargin{:})...
                        }...
            );
            ...
            handles.tab(sequence_tab).C3LoverDTolerance = uicontrol(...
                ...
                'callback'  , @(varargin)...
                        modify_settings('change_single_setting', ...
                                        'GAM.constraints.C3LoverD_tolerance',...
                                        [], ...
                                        varargin{:})...
            );
            uicontrol(...
        ...
        departure_panel = uipanel(...
            ...
            handles.tab(sequence_tab).launch_window.year(1) = uicontrol(...
                ...
                'callback'  , @(varargin){...
                        modify_settings('change_single_setting',...
                                        'departure.launch_window.year(1)',...
                                        [],...
                                        varargin{:});
                        callbacks('check_dates',...
                                  varargin{:})...
                        }...
            );
            handles.tab(sequence_tab).launch_window.month(1) = uicontrol(...
                ...
                'callback'  , @(varargin){...
                        modify_settings('change_single_setting',...
                                        'departure.launch_window.month(1)',...
                                        [],...
                                        varargin{:});
                        callbacks('check_dates',...
                                  varargin{:})...
            ...
            handles.tab(sequence_tab).launch_window.year(2) = uicontrol(...
                ...
                'callback'  , @(varargin){...
                        modify_settings('change_single_setting', ...
                                        'departure.launch_window.year(2)',...
                                        [],...
                                        varargin{:});
                        callbacks('check_dates',...
                                  varargin{:})...
            ...
            handles.tab(sequence_tab).launch_window.month(2) = uicontrol(...
                ...
                'callback'  , @(varargin){...
                        modify_settings('change_single_setting',...
                                        'departure.launch_window.month(2)',...
                                        [], ...
                                        varargin{:});
                        callbacks('check_dates',...
                                  varargin{:})...
        ...
        swingby_panel = handles.tab(sequence_tab).swingby_panel;
            ...
            % descriptions
            swingbypanel_uicontrol_defaults = {'parent', swingby_panel,...
                                               'units' , 'normalized'};

            swingbypanel_text_defaults = [swingbypanel_uicontrol_defaults, {...
                                          'style', 'text',...
                                          'horizontalalignment', 'left'}];

            swingbypanel_popup_defaults = [swingbypanel_uicontrol_defaults, {...
                                           'style'     , 'popup',...
                                           'background', environment.colors.edit_bgcolor}];

            swingbypanel_edit_defaults = [swingbypanel_uicontrol_defaults, {...
                                          'style'     , 'edit',...
                                          'background', environment.colors.edit_bgcolor}];

            swingbypanel_button_defaults = [swingbypanel_uicontrol_defaults, {...
                                            'style', 'pushbutton'}];
            ...
            for i = 1:4
                ...
                handles.tab(sequence_tab).GAM.number(i) = uicontrol(...
                    swingbypanel_text_defaults{:},...
                    'position', [x .90 .15 .1],...
                    'string'  , ['Swingby ' i_str]);

                handles.tab(sequence_tab).GAM.body(i) = uicontrol(...
                    swingbypanel_popup_defaults{:},...
                    'position', [x .90 .16 .06],...
                    ...
                    'callback', @(varargin){...
                            modify_settings('change_single_setting',...
                                            ...
                                            [],...
                                            varargin{:});
                            callbacks('switch_bodies',...
                                      i,...
                                      varargin{:})...
                            }...
                );
                handles.tab(sequence_tab).GAM.TOF_LB(i) = uicontrol(...
                    swingbypanel_edit_defaults{:},...
                    'position', [x .83 .16 .06],...
                    'callback', @(varargin)...
                            callbacks('change_TOF',...
                                      i,...
                                      varargin{:})...
                );
                ...
                handles.tab(sequence_tab).GAM.minalt(i) = uicontrol(...
                    swingbypanel_edit_defaults{:},...
                    'position', [x .65 .16 .06],...
                    'callback', @(varargin) ...                      % changing this one's settings
                            callbacks('change_minimum_altitude', ... % is an exceptional case:
                                      i,...
                                      'check',...
                                      varargin{:})...
                );
                ...
                handles.tab(sequence_tab).GAM.CompleteRevolutions(i) = uicontrol(...
                    ...
                    'callback', @(varargin)...
                            modify_settings('change_single_setting',...
                                            ['GAM.complete_revolutions(' i_str ')'],...
                                            [],...
                                            varargin{:})...
                );
            ...
            set(handles.tab(sequence_tab).GAM_jettison_group,...
                'SelectionChangeFcn', @(varargin) ...
                        modify_settings('change_single_setting',...
                                        'GAM.jettison',...
                                        handles.tab(sequence_tab).GAM.jettison, ...
                                        varargin{:})...
            );
            ...
            set([handles.tab(sequence_tab).GAM.number(1), ...
                 handles.tab(sequence_tab).GAM.body(1)],...
                 'enable', 'on');
        ...
        target_panel = uipanel(...
            ...
            target_panel_defaults = {'parent', target_panel,...
                                     'units' , 'normalized'};

            target_panel_text_defaults = [target_panel_defaults, {...
                                          'style'              , 'text',...
                                          'horizontalalignment', 'left'}];

            target_panel_popup_defaults = [target_panel_defaults, {...
                                          'style'     , 'popup',...
                                          'background', environment.colors.edit_bgcolor}];

            target_panel_edit_defaults = [target_panel_defaults, {...
                                         'style'     , 'edit',...
                                         'background', environment.colors.edit_bgcolor}];

            target_panel_button_defaults = [target_panel_defaults, {...
                                            'style', 'pushbutton'}];

            uicontrol(...
                target_panel_text_defaults{:},...
                'position'  , [.05 .885 .3 .06],...
                'string'    , 'Body:');
            handles.tab(sequence_tab).target.body = uicontrol(...
                target_panel_popup_defaults{:},...
                'position', [.35 .90 .6 .06],...
            ...
            handles.tab(sequence_tab).MPTargetButton = uicontrol(...
                target_panel_button_defaults{:},...
                'horizontalalignment', 'left',...
                'position', [.05 .60 .9 .06],...
                'enable'  , 'off',...
                'string'  , 'Minor planet...',...
                'callback', @(varargin) ...
                    callbacks('MP_target_body',...
                              varargin{:})...
            );
            % shortway/longway
            uicontrol(...
        ...
        handles.tab(sequence_tab).BatchOptimizeCheck = uicontrol(...
            ...
            'callback', @(varargin){...
                            modify_settings('change_single_setting',...
                                            ...
                                            [],...
                                            varargin{:});
                            callbacks('batch_optimization_check',...
                                      varargin{:})...
                            }...
        );
        handles.tab(sequence_tab).BatchOptimizeButton = uicontrol(...
            ...
            'enable'  , 'off',...
            'callback', @(varargin) ...
                    callbacks('batch_optimization_wrapper', ...
                              varargin{:})....
        );
    ...
    handles.tab(arrival_tab).usr_costfun_arrival(1) = uicontrol(...
        ...
        'callback', @(varargin){...
                modify_settings('change_single_setting', ...
                                'arrival.user_cost_function',...
                                [],...
                                varargin{:})
                      callbacks('arrival_costfunction_check',...
                                varargin{:})...
                }...
    );
    ...
    handles.tab(arrival_tab).arrival_constraints = arrival_constraint_panel;
        ...
        handles.tab(arrival_tab).MaxArrivalC3 = draw_arrival_elements(...
            ...
            'Max. C3 at arrival:',...
            @(varargin) modify_settings('change_single_setting',...
                                        'arrival.constraints.max_C3',...
                                        [],...
                                        varargin{:}),...
            'km�/s�',...
            true);
...
%             'Style'   , 'text');
    ...
    handles.tab(arrival_tab).post_processing_panel = uipanel(...
        ...
        handles.tab(arrival_tab).post_processing.check = uicontrol(...
            ...
            'callback' , @(varargin) {...
                    callbacks('enable_postprocessing',...
                              varargin{:});
                    modify_settings('change_single_setting', ...
    ...
    glob_panel = uipanel(...
        ...
        set(handles.tab(algorithms_tab).glob_opt_group,...
            'SelectionChangeFcn', @(varargin) ...
                    modify_settings('change_single_setting',...
                                    'optimize.global.optimizer',...
                                    handles.tab(algorithms_tab).global_optimizer,...
                                    varargin{:})...
        );
        ...
        low_approx_panel = uibuttongroup(...
            ...
            'Position', [0.53 0.02 0.47 0.86] ...
        );
    
            lt_defaults = {'Parent', low_approx_panel,...
                           'Units' , 'normalized'};
            lt_radio_detaults = [lt_defaults, {...
                                 'Style' , 'radiobutton'}];
    
            % Exposins
            handles.tab(algorithms_tab).LowThrustApproximation(1) = uicontrol(...
                lt_radio_detaults{:},...                
                'Position', [0.0625 0.56 0.80 0.23],...
                'String'  , 'Exponential Sinusoids');
            % Equinoctial elements
            handles.tab(algorithms_tab).LowThrustApproximation(2) = uicontrol(...
                lt_radio_detaults{:},...                
                'Position', [0.0625 0.30 0.77 0.30],...
                'String'  , 'Equinoctial elements',...
                'enable'  , 'off');
            % Parameters button
            uicontrol(...
                lt_defaults{:},...
                'Position', [0.0625 0.08 0.81 0.16],...
                'String'  , 'Parameters...',...
                'callback', @not_yet_done);
            
        set(low_approx_panel,...
            'SelectionChangeFcn', @(varargin)...
                modify_settings('change_single_setting', ...
                                'optimize.global.low_thrust_approximation',...
                                handles.tab(algorithms_tab).LowThrustApproximation,...
                                varargin{:})...
        );

        handles.tab(algorithms_tab).LowThrustApproximationGroup = low_approx_panel;
    ...
    obj_panel = uipanel(...
        ...
        handles.tab(algorithms_tab).MaxPayloadObjective = uicontrol(...
            'Parent'  , obj_panel,...
            'Units'   , 'normalized',...
            'Position', [0.06 0.80 0.895 0.11],...
            'String'  , 'Maximum payload mass',...
            'Style'   , 'checkbox',...
            'callback', @(varargin){...
                    modify_settings('change_single_setting', ...
                                    'optimize.objectives.max_mass', ...
                                    [], ...
                                    varargin{:});
                    callbacks('objectives_selection', ...
                              varargin{:})...
                    });
        ...
        handles.tab(algorithms_tab).MinTOFObjective = uicontrol(...
            'Parent'  , obj_panel,...
            'Units'   , 'normalized',...
            'Position', [0.06 0.67 0.91 0.11],...
            'String'  , 'Minimum Time of flight',...
            'Style'   , 'checkbox',...
            'callback', @(varargin){...
                    modify_settings('change_single_setting', ...
                                    'optimize.objectives.min_tof',...
                                    [],...
                                    varargin{:});
                    callbacks('objectives_selection',...
                              varargin{:})...
        ...
        % Minimum arrival speed
        handles.tab(algorithms_tab).MinArrivalSpeedObjective = uicontrol(...
            'Parent'  , obj_panel,...
            'Units'   , 'normalized',...
            'Position', [0.06 0.54 0.91 0.11],...
            'String'  , 'Minimum relative speed on arrival',...
            'Style'   , 'checkbox',...
            'enable'  , 'off',...
            'callback', @(varargin){...
                    modify_settings('change_single_setting', ...
                                    'optimize.objectives.min_arrival_speed',...
                                    [],...
                                    varargin{:});
                    callbacks('objectives_selection',...
                              varargin{:})...
        ...
        handles.tab(algorithms_tab).MinArrivalSpeedObjective = uicontrol(...
            'Parent'  , obj_panel,...
            'Units'   , 'normalized',...
            'Position', [0.06 0.41 0.91 0.11],...
            'String'  , 'Optimum approach angle for landing site',...
            'Style'   , 'checkbox',...
            'enable'  , 'off',...
            'callback', @(varargin){...
                    modify_settings('change_single_setting', ...
                                    'optimize.objectives.best_approach_angle',...
                                    [],...
                                    varargin{:});
                    callbacks('objectives_selection',...
                              varargin{:})...
        ...
        handles.tab(algorithms_tab).OtherObjectives.button = uicontrol(...
            ...
            'String'  , 'Load...',...
            'Style'   , 'pushbutton',...
            'callback', @(varargin) callbacks('select_costfcn_plugin', ...
                                              varargin{:}));
    ...
    loc_panel = uipanel(...
        ...
        'Position', [0.302 0.0209205020920502 0.6796875 0.479079497907949]);
        ...
        set(loc_opt_group, ...
            'SelectionChangeFcn', @(varargin)...
                    modify_settings('change_single_setting',...
                                    'optimize.local.optimizer', ...
                                    handles.tab(algorithms_tab).local_optimizer,...
                                    varargin{:})...
        );
        ...
        int_panel = uibuttongroup(...
            ...
            handles.tab(algorithms_tab).integrator(1) = uicontrol(...
                'Parent'  , int_panel,...
                'Units'   , 'normalized',...
                'Position', [0.0493827160493827 0.85 0.537037037037037 0.131428571428571],...
                'String'  , 'ODE113',...
                'Style'   , 'radiobutton');
            ...
            handles.tab(algorithms_tab).integrator(2) = uicontrol(...
                'Parent'  , int_panel,...
                'Units'   , 'normalized',...
                'Position', [0.0493827160493827 0.70 0.537037037037037 0.131428571428571],...
                'String'  , 'RKN8(6)',...
                'Style'   , 'radiobutton');
            ...
            handles.tab(algorithms_tab).integrator(3) = uicontrol(...
                'Parent'  , int_panel,...
                'Units'   , 'normalized',...
                'Position', [0.0493827160493827 .55 0.537037037037037 0.131428571428571],...
                'String'  , 'RKN12(10)',...
                'Style'   , 'radiobutton');

            % ODEX2
            handles.tab(algorithms_tab).integrator(4) = uicontrol(...
                'Parent'  , int_panel,...
                'Units'   , 'normalized',...
                'enable'  , 'off',...
                'Position', [0.0493827160493827 .40 0.537037037037037 0.131428571428571],...
                'String'  , 'ODEX2',...
                'Style'   , 'radiobutton');

            % GBS
            handles.tab(algorithms_tab).integrator(5) = uicontrol(...
                'Parent'  , int_panel,...
                'enable'  , 'off',...
                'Units'   , 'normalized',...
                'Position', [0.0493827160493827 .25 0.537037037037037 0.131428571428571],...
                'String'  , 'GBS',...
                'Style'   , 'radiobutton');
        ...
        set(int_panel,...
            'SelectionChangeFcn', @(varargin)...
                    modify_settings('change_single_setting',...
                                    'optimize.local.integrator', ...
                                    handles.tab(algorithms_tab).integrator,...
                                    varargin{:})...
        );
        ...
        loc_method_group = uibuttongroup(...
            'Parent'  , loc_panel,...
            'Title'   , 'Low-thrust optimization method',...
            'Position',[0.66 0.02 0.32 0.91] ...            
        );
                ...
                uicontrol(...
                    'Parent'  , loc_method_group,...
                    'Units'   , 'normalized',...
                    'Position', [0.05 0.06 0.92 0.14],...
                    'String'  , 'Parameters...',...
                    'callback', @not_yet_done);
                
        set(loc_method_group, ...
            'SelectionChangeFcn', @(varargin)...
                   modify_settings('change_single_setting',...
                                   'optimize.local.method', ...
                                   handles.tab(algorithms_tab).low_thrust_optimization_method,...
                                   varargin{:})...
        );

    handles.tab(algorithms_tab).second_order_panel = loc_panel;
    handles.tab(algorithms_tab).second_order_panel_low_thrust_method_group = loc_method_group;

    % Ephemerides panel
    eph_panel = uibuttongroup(...
        'Parent'  , handles.tab(algorithms_tab).panel,...,...
        'Title'   , 'Ephemerides generator',...
        'Position', [0.63 0.53 0.35 0.45] ...        
    );

        ephradios_defaults = {'Parent', eph_panel,...
                              'Units' , 'normalized',...
                              'Style' , 'radiobutton'};

         % JPL development ephemerides, 405
         handles.tab(algorithms_tab).Ephemerides(1) = uicontrol(...
            ephradios_defaults{:},...
            'Position', [0.10 0.80 0.82 0.12],...
            'String'  , 'JPL / DE405');
        
         % JPL development ephemerides, 421
         handles.tab(algorithms_tab).Ephemerides(2) = uicontrol(...
            ephradios_defaults{:},...
            'Position', [0.10 0.57 0.82 0.12],...
            'String'  , 'JPL / DE421');
        
        if ~environment.have_DE421_MEX
            set(handles.tab(algorithms_tab).Ephemerides(2),...
                'enable', 'off');
        end
        ...
        handles.tab(algorithms_tab).Ephemerides(3) = uicontrol(...
            ephradios_defaults{:},...
            'Position', [0.10 0.33 0.82 0.12],...
            'String'  , 'Kepler (direct)');
        
        % Kepler (5th order polynomial interpolation)
        handles.tab(algorithms_tab).Ephemerides(4) = uicontrol(...
            ephradios_defaults{:},...
            'Position', [0.10 0.10 0.82 0.12],...
    ...
    set(eph_panel, ...
        'SelectionChangeFcn', @(varargin)...
                modify_settings('change_single_setting',...
                                'optimize.ephemerides', ...
                                handles.tab(algorithms_tab).Ephemerides,...
                                varargin{:})...
    );
...
%        output_tab_tab(BATCH_optimization) =
    ...
    % Initially, everything is disabled because high-thrust is selected
    callbacks('set_propulsiondependent_pwr_controls',  false);
    callbacks('set_propulsiondependent_algo_controls', false);
...
% close main window
function close_main_window(varargin) %#ok<VANUS>

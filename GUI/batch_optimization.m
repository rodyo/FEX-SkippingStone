...
function [accept, settings] = batch_optimization(varargin)
    ...
    set(explanatory_text,...
        'string', textwrap(explanatory_text, help_string));
    ...
    function abbreviations = abbreviate_names(which_ones, letters)
        ...
        % defaults
        if (nargin == 0)
            which_ones = 1:numel(body_names);
            letters = 1;
        end

        % form abbreviations
        abbreviations = cellfun(@(x)x(1:letters), ...
                                body_names(which_ones),...
                                'UniformOutput', false);
        % check for dupes
        for ii = 1:numel(abbreviations)
            ...
            if (nnz(equals) > 1)
                abbreviations(equals) = abbreviate_names(find(equals), letters+1); end
        end
    ...
    function combinations = all_combinations(varargin)
        ...
        for ii = 1:numel(BATCH.bodies)
            indices(ii) = find(strcmpi(body_names, BATCH.bodies{ii}), 1); end
